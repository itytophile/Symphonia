...
struct DecoderInstance {
    format: BlockingFormatReader<Box<dyn AsyncFormatReader>>,
...
fn run_test(path: &str, opts: &TestOptions, result: &mut TestResult) -> Result<()> {
    ...
    let ref_mss = MediaSourceStream::new(
    ...
    let tgt_mss = MediaSourceStream::new(File::open(Path::new(path))?, Default::default());
